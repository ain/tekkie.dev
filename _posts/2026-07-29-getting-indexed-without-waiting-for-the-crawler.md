...
last_modified_at: 2026-07-29 22:10:00 +02:00
---
...
- **IndexNow** — an open protocol, originally from Microsoft, now with several participants sharing submissions between them. You POST a list of URLs with a key.
- **Google Indexing API** — Google's own endpoint, authenticated with a service account tied to a verified property.

Neither is hard in principle. Both are annoying in practice: one wants a key file published at a specific path on your own host, the other wants an RS256-signed JWT exchanged for an access token, and both have opinions about quotas that only surface as an HTTP 429 halfway through a batch.
...
Sitemap parsing is deliberately strict. A sitemap must be valid XML in the [sitemaps.org](https://www.sitemaps.org/protocol.html){:rel="nofollow external"} format with at least one `urlset > url > loc`, and every `loc` an absolute `http` or `https` URL. Namespace prefixes are fine. Only that exact nesting is read, which means the `<loc>` inside an image or video extension — nested a level deeper — is correctly ignored rather than submitted as if it were a page. Sitemap index files are not supported and say so rather than silently yielding nothing.
...
`--credentials` is required even with `--dry-run`, which looks pedantic until you consider that a pre-flight check passing on input a real run would reject is worth nothing. A credentials file is read and its JSON parsed during a dry run, since neither step touches the network.

There is a nice bit of judgement in how that argument is interpreted: a value is treated as a file if it exists on disk, and as a token otherwise — but a value that *looks* like a path and does not exist is an error rather than a token. Guessing "token" there would surface much later as a baffling 401.
...
Verbose output is designed to be pasteable as-is. The `Authorization` header is always redacted.
...
- `2` for invalid arguments or an unreadable sitemap. Enough to branch on in a deploy script
